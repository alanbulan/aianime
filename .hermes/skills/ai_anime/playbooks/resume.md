...
- 若用户本会话已选 / 现在表示要**逐步确认模式** → `Read references/run-modes.md` 模式一，
  从断点起**每个写操作步骤前都调用 `question`，一次只推进一步**。
- 若用户选**连续自动推进模式** → 调用一次 `ai_anime_run_production_workflow`，由同一后端父任务从断点恢复；已有 `production_workflow` queued/running 时只等待该父任务，不提交第二个。
- 若用户只说「继续」且**未指定过模式** → 继承最近一次明确目标。最近目标是完整整集、跨阶段推进，或上一 `production_workflow` 已失败/取消时，重新调用一次完整生产父任务从断点恢复；最近目标明确是局部操作或逐步确认时，才推进一个步骤。

先根据用户请求判断恢复模式：

- **用户只是在问进度 / 想看当前做到哪一步**
  - 展示进度表
  - 必要时通过 `question` 选择交互模式（每步确认/自动推进）
  - 暂不直接推进
- **用户已经明确要求继续 / 恢复 / 从这里开始**
  ...
  - 如果最近的完整生产父任务为 failed/cancelled，用户的“继续”表示恢复该父目标，只能重新提交完整生产父任务；`next_step` 仅用于说明断点，不得据此切换为局部写工具
...
必须通过 `question` 确认后才调 `pool-select` 并传 `force=true`。禁止 try→fail→force 模式。
