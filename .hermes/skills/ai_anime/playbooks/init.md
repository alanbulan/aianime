...
- **404**：当前会话绑定的项目不存在或不可访问 → 停止，提示用户先在前端创建/打开项目；AI anime 助手不调用 `POST /projects`
- **200 + 各阶段全 false**：项目已存在但未摄入 → 从 Step 1（获取小说文件）开始
- **200 + 部分完成**：实际上应走 resume.md；如误路由到此，退回按 `next_step` 定位

## 入口决策树

> 进入本 playbook 的前提是当前消息已经带有剧本文档附件，或前端已注入 `[AI_ANIME_INGEST_AUTOMATION]` / 明确摄入上下文。若用户只是用聊天文字要求“创建/生成/写剧本”，不得进入下面流程；直接告知只能通过“故事导入”上传剧本文档。

```
1. 获取小说文件
   当前消息必须已有剧本文档附件或故事导入摄入上下文
   ├─ 有 → 使用前端提供的上传/摄入上下文
   └─ 没有 → 停止，提示通过“故事导入”上传剧本文档
...
**步骤详情**：见 `references/pipeline-details.md` Steps 1-7。项目创建由前端/系统完成，不属于AI anime 助手步骤。
...
- 如果为空，先根据角色名、性别、年龄段、description 生成一句具体面部特征描述，调用 `ai_anime_update_character_face_prompt` 写入。
