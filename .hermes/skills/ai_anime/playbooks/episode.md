...
CP2 判定：本集脚本、身份、场景/道具规划和引用图片齐全；完整成片目标还必须先补齐本集实际需要的角色/身份、旁白及视频参考声线。只做静态分镜时不要求声线。当前后端没有 `anchor_image_url` 契约。

## 步骤要点

**整集制作入口**：
- 用户要求“完成第 N 集 / 自动生成整集 / 做成片”时，只调用一次 `ai_anime_run_production_workflow(episodes=[N])` 并等待父任务。
- 父任务会从持久化断点补齐脚本、生产模型前置、视觉资产、声线与配音模型前置，再依次执行草图、检测、首帧、全局优化、声线复检、最终提示词、音频、逐 beat 视频与合成；助手不得再逐阶段编排。
- `pipeline/status` 的 `voice_assets` / `voice_asset_issues` 表示本集声线是否就绪及缺失原因；不得把视觉资产齐全当成完整生产资产已齐全。遇到此断点，完整生产仍恢复同一个父工作流，不用批量生成音频来冒充声线准备，也不要求填满未被本集使用的所有声线槽位。
...
- 剧本生成完成后补齐道具与场景参考图；完整生产还要先补齐本集所需声线并复检，再进入草图
...
- 完成判定：本集视觉资产及绑定齐全；完整生产还需要本集所需声线就绪。不要要求 `anchor_image_url`，也不要把只做静态分镜的请求扩大为声线生成。
...
**局部音频更新**：
- 当用户修改 beat 的 `audio_type`、`speaker` 或 `narration_segment` 台词文本时，
  必须按固定顺序执行：
...
用户回复：
- "继续" / "ok" → 仅在当前明确处于逐步确认模式时推进当前 `next_step` 的一个任务；若是在恢复失败/取消的完整生产父任务，则重新调用一次 `ai_anime_run_production_workflow` 从持久化断点继续，不改走局部工具
