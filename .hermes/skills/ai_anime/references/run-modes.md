...
- 完整生产在草图前先补齐本集视觉资产、角色/身份与旁白声线及视频所需参考声线，再验收放行；已有可用声线复用，未使用角色和年龄槽位不阻塞。实际顺序为草图 → 检测 → 首帧 → 全局优化 → 声线复检 → 最终视频提示词 → 配音 → 视频 → 合成。
- `pipeline/status.next_step=voice_assets` 表示本集声线前置未通过，读取 `voice_asset_issues` 解释原因，再由同一父工作流恢复。不得忽略此项继续生成草图；用户仅要求静态分镜时，单步图片入口仍只检查视觉前置。
