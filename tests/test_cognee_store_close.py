...
async def test_graph_snapshot_reads_the_project_dataset_database(monkeypatch):
    ...
    def fake_set_database_context(dataset_id, owner_id):
        calls.append(("context.set", dataset_id, owner_id))
        return FakeDatasetContext()
    ...
    monkeypatch.setattr(
        ...
        fake_set_database_context,
    )

    store = CogneeStore.__new__(CogneeStore)
    store.dataset_name = "project-dataset"
    ...
    assert calls == [
        "project.context",
        ("dataset.lookup", "project-dataset", "user-id"),
        ("context.set", "dataset-id", "owner-id"),
        "context.enter",
        "graph.read",
        "context.exit",
    ]
