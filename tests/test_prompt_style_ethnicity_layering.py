...
def test_character_default_ethnicity_instruction_allows_explicit_foreign_descriptions():
    ...
    assert (
        "only when the character description and reference images do not specify"
        in instruction
    )
    ...
    for path in files:
        ...
        for literal in forbidden_literals:
            assert literal not in source, (
                f"{path} still contains hard ethnicity constraint {literal!r}"
            )
...
def test_anime_preset_contains_only_rendering_style():
    path = Path("src/ai_anime/styles/presets/anime.json")
    data = json.loads(path.read_text())
...
def test_post_apocalyptic_preset_does_not_force_human_or_weathered_subjects():
    path = Path("src/ai_anime/styles/presets/post_apocalyptic.json")
    data = json.loads(path.read_text())
    instructions = data["style_instructions"].lower()
