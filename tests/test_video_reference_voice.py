...
class _FakeGenerator:
    ...
    async def generate(self, *, prompt, audio_url, output_path, emotion_prompt=""):
        ...
        return TTSResult(
            success=True, audio_path=str(output_path), duration_seconds=1.25
        )
...
def test_dialogue_text_extracts_spoken_quote_from_attribution():
    from ai_anime.modules.production.infrastructure.video_reference_voice import dialogue_text
...
def test_video_audio_type_preserves_silence():
    from ai_anime.modules.production.infrastructure.video_reference_voice import (
        normalize_video_audio_type,
    )

    assert (
        normalize_video_audio_type({"audio_type": "silence", "speaker": ""})
        == "silence"
    )


@pytest.mark.asyncio
async def test_video_reference_voice_clone_uses_identity_reference_before_character(tmp_path):
    from ai_anime.modules.production.infrastructure.video_reference_voice import (
        generate_video_dialogue_audio,
    )
    ...
    result = await generate_video_dialogue_audio(
    ...
    assert (
        project_dir / "audio" / "ep001" / "beat_03.mp3"
    ).read_bytes() == b"beat-audio"


@pytest.mark.asyncio
async def test_video_reference_voice_clone_sends_only_spoken_text_to_fal(tmp_path):
    from ai_anime.modules.production.infrastructure.video_reference_voice import (
        generate_video_dialogue_audio,
    )
...
@pytest.mark.asyncio
async def test_video_reference_voice_clone_falls_back_to_character_default(tmp_path):
    ...
    assert generator.calls and generator.calls[0]["audio_url"].endswith(
        "voice_sample.wav"
    )


@pytest.mark.asyncio
async def test_video_reference_voice_clone_loads_default_identity_voice_file_when_path_not_persisted(
    tmp_path,
):
    from ai_anime.modules.asset_world.public import CharacterIdentity, NovelCharacter
    from ai_anime.modules.production.infrastructure.video_reference_voice import (
...
@pytest.mark.asyncio
async def test_video_reference_voice_clone_uses_age_group_preset(tmp_path):
...
@pytest.mark.asyncio
async def test_video_reference_voice_clone_styling_variant_uses_character_default(tmp_path):
...
@pytest.mark.asyncio
async def test_video_reference_voice_clone_returns_none_for_narration_beat(tmp_path):
    from ai_anime.modules.production.infrastructure.video_reference_voice import (
        generate_video_dialogue_audio,
    )
...
def test_video_reference_same_voice_beats_group_by_identity_speaker():
    from ai_anime.modules.production.infrastructure.video_reference_voice import (
...
@pytest.mark.asyncio
async def test_video_reference_voice_batch_generates_only_missing_same_identity_beats(
    tmp_path,
):
    from ai_anime.modules.production.infrastructure.video_reference_voice import (
        beat_audio_path,
        generate_video_dialogue_audio_for_voice,
    )
    ...
    generator = _FakeGenerator()
    result = await generate_video_dialogue_audio_for_voice(
        beats=beats,
        speaker="谢铮_幼年时期",
        episode=1,
        store=_FakeStore(project_dir, _characters()),
...
def test_resolve_narrator_source_first_person_falls_back_to_character_default(tmp_path):
    ...
    project_dir.mkdir()
    default_audio = (
        project_dir / "assets" / "characters" / "陆辰" / "voices" / "voice_default.mp3"
    )
...
def test_resolve_narrator_source_first_person_missing_narrator_main(tmp_path):
    ...
    project_dir.mkdir()
    store = _FakeStore(
        project_dir, [NovelCharacter(name="苏曼", gender="女", is_main=False)]
    )
...
def test_resolve_narrator_source_third_person_reports_unconfigured_voice(tmp_path):
    ...
    assert resolution.audio_path is None
    assert resolution.error == (
        "项目解说人声线未配置，请通过文字设计、已有声线、上传或录制完成配置"
    )
...
def test_resolve_narrator_source_third_person_reports_hash_read_failure(
    ...
    import ai_anime.modules.production.infrastructure.video_reference_voice as voice_clone
