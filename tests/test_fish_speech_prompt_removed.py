...
def test_fish_speech_prompt_field_and_agent_are_removed() -> None:
    sources = {
        ...
        "cognee store": Path(
            "src/ai_anime/modules/knowledge_graph/infrastructure/store.py"
        ).read_text(encoding="utf-8"),
        "sketch edit tasks": Path(
            "src/ai_anime/modules/verification/infrastructure/sketch_edit_tasks.py"
        ).read_text(encoding="utf-8"),
    }
...
def test_fish_audio_tts_path_is_removed() -> None:
    tts_generator = Path(
        "src/ai_anime/modules/production/infrastructure/media_generation/tts_generator.py"
    ).read_text(encoding="utf-8")
