...
def resolve_user_project_id_from_context(project_dir: Path) -> tuple[str, str]:
    ...
    username = (
        os.environ.get("AI_ANIME_USERNAME")
        or fallback_env.get("AI_ANIME_USERNAME")
        or ""
    ).strip()
    project_id = (
        os.environ.get("AI_ANIME_PROJECT_ID")
        or fallback_env.get("AI_ANIME_PROJECT_ID")
        or ""
    ).strip()
    if not username or not project_id:
        raise RuntimeError(
            "AI_ANIME_USERNAME / AI_ANIME_PROJECT_ID is not configured in env or "
...
def resolve_project_id_from_context(project_dir: Path) -> str:
    ...
    if not project_id:
        raise RuntimeError(
            "AI_ANIME_PROJECT_ID is not configured in env or .claude/settings.local.json"
        )
    return project_id
