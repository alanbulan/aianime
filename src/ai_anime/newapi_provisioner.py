...
def get_provisioner_config(
    ...
    *,
    ...
    return NewApiProvisionerConfig(
        ...
        relay_token_name=(
            os.environ.get("NEWAPI_RELAY_TOKEN_NAME", "ai-anime-ce-runtime").strip()
            or "ai-anime-ce-runtime"
        ),
    )
...
def build_channel_payload(
    *,
    ...
    if preset:
        channel_name = f"ai-anime-{provider_key}"
    else:
        channel_name = (name or f"ai-anime-type-{channel_type}").strip()
    channel = {
        "name": channel_name,
        ...
        "settings": "{}",
        "other": "",
        "remark": f"created by AI anime CE provisioner for {provider_label}",
    }
    if not channel["key"]:
...
def remove_model_keys_from_other_dc_channels(
    ...
    *,
    ...
    for channel in list_channels(cfg, admin):
        ...
        if not name.startswith("ai-anime-") or _is_same_channel(channel, target_channel):
            continue
