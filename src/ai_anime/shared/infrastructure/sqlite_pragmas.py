...
_PRAGMAS_BEFORE_JOURNAL = (
    # Switching journal mode may itself need a lock, so install the wait
    # policy before attempting it during coordinated schema bootstrap.
    ("busy_timeout", "10000"),
)
...
def litestream_enabled() -> bool:
    """Return whether AI_ANIME_LITESTREAM_ENABLED is truthy."""

    return os.environ.get("AI_ANIME_LITESTREAM_ENABLED", "").strip().lower() not in (
        "",
        "0",
        "false",
        "no",
