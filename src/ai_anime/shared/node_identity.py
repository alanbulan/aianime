...
stripped before use, so whitespace-only AI_ANIME_WORKER_ID falls back to disk/hostname.
"""
...
logger = logging.getLogger("ai_anime.shared.node_identity")
...
def resolve_worker_id() -> str:
    ...
    env_value = os.environ.get("AI_ANIME_WORKER_ID", "").strip()
    if env_value:
        _resolved_worker_id = env_value
        return _resolved_worker_id

    disk = Path(
        os.environ.get("AI_ANIME_WORKER_ID_DISK_PATH", "").strip()
        or "/var/lib/ai_anime/worker_id"
    )
    try:
        if disk.exists():
    ...
    try:
        ...
        logger.warning(
            ...
            "Set AI_ANIME_WORKER_ID explicitly in prod.",
            disk,
            exc,
        )
