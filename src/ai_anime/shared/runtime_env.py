...
def task_backend() -> str:
    _load_env()
    return os.environ.get("AI_ANIME_TASK_BACKEND", "celery").strip() or "celery"


def edition() -> str:
    _load_env()
    return os.environ.get("AI_ANIME_EDITION", "").strip().lower()
...
def is_ce_effective() -> bool:
    return edition() == "ce" and not os.environ.get("AI_ANIME_CONTROL_PLANE_DSN", "").strip()
...
def cookie_secure() -> bool:
    ...
    # opt out via ``AI_ANIME_COOKIE_SECURE=0`` — the admin cookie is
    # also SameSite=Strict, so a Secure cookie on plain http
    # localhost is silently dropped by the browser.
    return _env_bool("AI_ANIME_COOKIE_SECURE", default=True)
