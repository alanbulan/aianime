...
def restore_cell(
    ...
    prefix = (
        ...
        or f"backup/{os.environ['BACKUP_ENV_NAME']}/{os.environ['AI_ANIME_WORKER_ID']}"
    ).strip("/")
    state_dir = Path(os.environ["AI_ANIME_STATE_DIR"])
    ...
    if include_output:
        out_root = Path(os.environ["AI_ANIME_OUTPUT_DIR"]) / user / f"{project}.restored"
        plans.append(
            [
                "rclone",
                "copy",
