...
def _open_snapshot_source_fd(source_path: Path) -> int:
    ...
    if source_path.is_symlink():
        raise OSError(f"snapshot source is a symlink: {source_path}")
    ...
    if handle == wintypes.HANDLE(-1).value:
        ...
        raise OSError(error, ctypes.FormatError(error), str(source_path))
    try:
...
def _copy_stable_file(
    ...
    *,
    ...
    try:
        source_fd = _open_snapshot_source_fd(source_path)
...
def _canvas_id_for_hot_path(relative_path: Path) -> str | None:
    ...
    if directory_name == "canvas_idempotency" and filename.endswith(".json"):
        ...
        return None
    return candidate if is_valid_creative_canvas_id(candidate) else None
    ...
    for project_rel, paths_by_canvas in sorted(canvas_paths_by_project.items()):
        ...
        for canvas_id, relative_paths in sorted(paths_by_canvas.items()):
            ...
            if not has_canvas_record:
                continue
            with creative_canvas_write_lock(
                project_dir,
                canvas_id,
...
def main() -> int:
    ...
    state_dir = _required_source_dir("AI_ANIME_STATE_DIR")
    sync_output = os.environ.get("BACKUP_SYNC_OUTPUT") == "1"
    output_dir = _required_source_dir("AI_ANIME_OUTPUT_DIR") if sync_output else None
