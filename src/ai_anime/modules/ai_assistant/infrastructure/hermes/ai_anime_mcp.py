"""MCP bridge for AI anime tools.

Hermes uses ``.hermes/plugins/ai_anime`` directly. Claude, Codex, and other
MCP-speaking agents use this stdio server to call that same toolset without
duplicating AI anime API wrappers.
"""
...
def _load_ai_anime_plugin() -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load AI anime plugin from {plugin_path}")
...
PLUGIN = _load_ai_anime_plugin()
TOOLS = _tool_index(PLUGIN)
SERVER = Server("ai_anime", version="0.1.0")
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[types.TextContent]:
    item = TOOLS.get(name)
    if item is None:
        raise ValueError(f"unknown AI anime tool: {name}")
