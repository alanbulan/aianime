...
# AI anime repo root: src/ai_anime/security/sandbox_wrap.py → parents[3]
AI_ANIME_ROOT = Path(__file__).resolve().parents[3]
SANDBOX_PROFILES_DIR = AI_ANIME_ROOT / "deploy" / "sandbox"
...
def _data_dir(kind: str) -> Path:
    env = os.environ.get(f"AI_ANIME_{kind.upper()}_DIR", "").strip()
    if env:
        return Path(env).expanduser()
    return AI_ANIME_ROOT / kind
...
class SandboxSpec:
    ...
    the AI anime layout (state/{user}/, output/{user}/, runtime/{user}/).
    """
    ...
    def shared_read_paths(self) -> list[Path]:
        ...
        paths: list[Path] = [
            ...
            AI_ANIME_ROOT / "src",
            AI_ANIME_ROOT / "integrations",
            AI_ANIME_ROOT / ".hermes",  # repo-pinned Hermes skills
            AI_ANIME_ROOT / ".venv",  # AI anime's main venv for skill scripts
        ]
        return paths
...
def wrap_command(cmd: list[str], spec: SandboxSpec) -> list[str]:
    ...
    - AI_ANIME_ENV=production → raise (must sandbox in prod).
...
def build_macos_profile(spec: SandboxSpec) -> str:
    ...
    parts: list[str] = ["\n;; ===== AI anime per-user dynamic policy =====\n"]

    # Hermes workers need outbound network access for the configured LLM
    # provider and AI anime API calls. File writes and agent-side tools remain
...
def _fallback_or_raise(cmd: list[str], reason: str) -> list[str]:
    if os.environ.get("AI_ANIME_ENV", "").lower() == "production":
