...
process.stdin.on('end', () => {
  const maxOps = Number(process.env.AI_ANIME_JS_MAX_OPS || '200000');
  const operations = [];
...
SYSTEM_PROMPT = """You are a Minecraft-style block world builder for AI anime.
...
def execute_build_code_with_node(code: str, *, timeout_seconds: int = 8) -> list[dict[str, Any]]:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "AI_ANIME_JS_MAX_OPS": "200000"},
        )
...
def world_to_scene_spec(
    *,
    ...
    return {
        ...
        "generator": (
            "ai_anime.modules.asset_world.infrastructure.director_world."
            "block_world_builder"
        ),
        "grid": {
            "block_size_m": 0.45,
