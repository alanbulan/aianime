"""Cooperative cancellation use cases for project tasks."""
...
from ai_anime.modules.task_execution.application.ports import CancellationStore
...
async def is_cancel_requested(
    cancellation_store_provider: CancellationStoreProvider,
    *,
    project_id: str,
    task_type: str,
    episode: int,
    ...
    return await cancellation_store_provider().is_cancel_requested(
...
async def await_envelope_with_cancel_watch(
    ...
    *,
    ...
    return await await_with_cancel_watch(
        coro,
        cancellation_store_provider=cancellation_store_provider,
        **fields,
        deadline_monotonic=_optional_float(envelope.get("__deadline_monotonic")),
        timeout_seconds=_optional_int(envelope.get("__timeout_seconds")),
    )
...
def raise_if_envelope_cancel_requested(
    ...
    *,
    ...
    if not fields["task_id"]:
        return
    if asyncio.run(is_cancel_requested(cancellation_store_provider, **fields)):
        raise TaskCancelled()
