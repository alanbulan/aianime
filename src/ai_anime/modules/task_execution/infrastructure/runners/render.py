...
from ai_anime.modules.task_execution.infrastructure.runners.sketch import (
...
async def _run_batch_render_async(
    ...
    log(f"并发执行 {len(prepared_requests)} 个图片请求...", progress=0.2)

    def on_status(state):
        log(f"并发请求状态: {state}", progress=0.5)

    results = await generator.generate_prepared_requests(
    ...
    for mk, grids_meta in metadata_by_mode.items():
        metadata = {"mode": mk, "grids": grids_meta, "concurrent_requests": True}
