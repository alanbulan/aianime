"""Creative Canvas image reverse-prompt adapter."""
...
async def reverse_prompt_from_image(*, image_path: Path) -> str:
    _model, prompt_text = await call_creative_canvas_vision_model(
...
class VisionCreativeCanvasReversePromptGenerator:
    async def generate(self, image_path: Path) -> str:
        return await reverse_prompt_from_image(image_path=image_path)
