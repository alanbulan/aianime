...
external automation clients) to drive the Director 3D editor through Bash.

Subcommands:
- catalog: dump scene catalog JSON for a world.json
- query: GET current state from /__ai_anime/director-state-query
- push: POST a director command via /__ai_anime/director-push
...
def _query_state(base_url: str, session: str, frame_id: str = "") -> dict:
    ...
    try:
        resp = httpx.get(
            f"{base_url}/__ai_anime/director-state-query",
            params=params,
            timeout=15.0,
        )
...
def _push_command(
    ...
    try:
        resp = httpx.post(
            f"{base_url}/__ai_anime/director-push",
            json=body,
            timeout=15.0,
        )
        resp.raise_for_status()
...
def cmd_catalog(args: argparse.Namespace) -> None:
    from ai_anime.skills.director_scene_catalog import load_scene_catalog
...
def cmd_pause(args: argparse.Namespace) -> None:
    try:
        resp = httpx.post(
            f"{args.base_url}/__ai_anime/director-pause",
