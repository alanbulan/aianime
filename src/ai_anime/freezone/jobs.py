...
- `provider` / `model` / `quality` get threaded into
  `get_grid_generation_config(provider_override=, model_override=)` for the
          image generation/edit path so the caller can pick the supported AI anime
...
async def run_freezone_gen(
    *,
    ...
    Routes through nanobanana_grid for the supported AI anime providers.
    """
...
async def run_freezone_upscale(
    *,
    ...
    from ai_anime.generators.image_generator import create_image_generator
