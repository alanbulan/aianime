...
from ai_anime.api import OPENAPI_TAGS, api_router, register_verification_routes
...
def create_app() -> FastAPI:
    ...
    application = FastAPI(title="AI anime API", openapi_tags=OPENAPI_TAGS)
    ...
    # 原生/便携部署(无 nginx)时由后端直接伺服 SPA:设 AI_ANIME_FRONTEND_DIST
    # 指向前端构建产物目录才挂载;Docker/EE 路径不设该变量,行为不变。
    # 挂在所有路由之后,/api、/healthz、/static 仍然优先命中。
    frontend_dist = os.environ.get("AI_ANIME_FRONTEND_DIST", "").strip()
