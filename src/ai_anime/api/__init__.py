"""AI anime Studio - REST API 模块。

提供 /api/v1/ 端点，供 AI anime 桌面端与外部自动化客户端调用。
"""
...
_api_logger = logging.getLogger("ai_anime.api")
...
from ai_anime.api.routes import (  # noqa: E402
    assets,
    auth,
    characters,
    chat,
...
if not runtime_env.is_ce_effective():
    for ep in entry_points(group="ai_anime.api_routes"):
        ep.load()(api_router)
