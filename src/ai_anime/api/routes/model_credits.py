...
from ai_anime.ports import get_credit_quote
...
def _fixed_image_cost_model(kind: str) -> str:
    if kind == "prop_reference":
        from ai_anime.generators.nanobanana_prop import resolve_prop_reference_image_model

        return resolve_prop_reference_image_model()
    if kind == "scene_master":
        from ai_anime.generators.scene_reference_images import resolve_scene_reference_image_model
...
def _image_selection_cost_model(selection: str) -> str:
    ...
    from ai_anime.config import IMAGE_GENERATION_SELECTIONS, character_image_selection_options
