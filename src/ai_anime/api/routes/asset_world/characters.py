...
async def update_character(
    ...
    try:
        data = await character_catalog_use_cases().update_character(
            repository=store,
            character_name=name,
            command=UpdateCharacterCommand(fields=body.model_dump(exclude_none=True)),
        )
    except CharacterCatalogRejected as exc:
        return {"ok": False, "error": str(exc)}
    return {"ok": True, "data": data}
...
async def design_missing_character_voices(
    ...
    resolved = await resolve_project_scope(project, user, required_role="editor")
    try:
        scheduled = await character_task_use_cases().schedule_voice_design(
    ...
    return {
        ...
        **scheduled.as_dict(),
        "agent_instruction": (
            "角色声线设计已进入任务队列；使用返回的 task_key 查询结果。"
            "本接口只生成并绑定声线，不会启动完整生产流程或整集配音。"
        ),
    }
...
async def upload_character_voice_sample(
    ...
    try:
        ...
        if not is_supported_voice_sample(filename):
            raise InvalidCharacterVoiceInput(SUPPORTED_VOICE_SAMPLE_MESSAGE)
        content = await file.read()
        created_voice, source_path = await call_blocking(
...
async def record_character_voice_sample(
    ...
    try:
        ...
        content, extension = await call_blocking(
            voice_use_cases.decode_recording,
            body.data_url,
        )
        created_voice, source_path = await call_blocking(
            _create_reusable_character_voice,
...
async def update_identity(
    ...
    try:
        data = await character_identity_use_cases().update_identity(
            ...
            character_name=name,
            identity_id=identity_id,
            command=UpdateIdentityCommand(fields=body.model_dump(exclude_none=True)),
        )
    except CharacterCatalogRejected as exc:
        return {"ok": False, "error": str(exc)}
    return {"ok": True, "data": data}
...
async def generate_single_portrait(
    ...
    """兼容入口；角色肖像统一提交后台任务。"""
    return await generate_single_portrait_async(project, name, body, user)
...
async def generate_identity_portrait(
    ...
    """兼容入口；身份 Portrait 统一提交后台任务。"""
    return await generate_identity_portrait_async(
        project,
        name,
        identity_id,
        body,
...
async def generate_identity_image(
    ...
    """兼容入口；身份参考图统一提交后台任务。"""
    return await generate_identity_image_async(
        project,
        name,
        identity_id,
        body,
