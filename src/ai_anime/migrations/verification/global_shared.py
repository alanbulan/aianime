...
Usage:
    uv run python -m ai_anime.modules.verification.presentation.cli.migrate_to_global_shared
    uv run python -m ai_anime.modules.verification.presentation.cli.migrate_to_global_shared --dry-run
"""
...
async def _merge_verification_db(src: Path, dst: Path, dry_run: bool) -> tuple[int, int]:
    ...
    async with aiosqlite.connect(str(dst)) as db_dst:
        await run_verification_registry_migrations(db_dst)
...
async def _merge_training_db(src: Path, dst: Path, dry_run: bool) -> dict[str, int]:
    ...
    async with aiosqlite.connect(str(dst)) as db_dst:
        await run_training_db_migrations(db_dst)
