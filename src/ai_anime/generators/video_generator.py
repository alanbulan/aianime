...
class VideoBackend(Enum):
    ...
    COMFYUI = "comfyui"  # Local ComfyUI service
...
class HuimengVideoGenerator(VideoGeneratorBase):
    ...
    def __init__(
        ...
        from ai_anime.generators.huimengi import HuimengiTaskClient
    ...
    def _to_upload_url(
        ...
        *,
        ...
        if os.path.exists(text):
            if require_http_url:
                from ai_anime.utils.oss_client import presign_or_upload_output
...
class NewApiVideoGenerator(VideoGeneratorBase):
    ...
    def _default_generate_audio_for_model(cls, model: str) -> bool:
        from ai_anime.config import NEWAPI_VIDEO_AUDIO_MODELS
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("AI anime API key must be set for AI anime API video generation")
            ...
            async with session.post(url, json=payload, headers=self.headers) as resp:
                ...
                if resp.status < 200 or resp.status >= 300:
                    ...
                    raise NewApiVideoError(
                        f"AI anime API submit failed: HTTP {resp.status} - {text}",
                        request_id=request_id,
                    )
                try:
                    data = json.loads(text)
                    ...
                    raise RuntimeError(f"AI anime API submit returned invalid JSON: {text}") from exc
        ...
        async with aiohttp.ClientSession(timeout=self._client_timeout()) as session:
            async with session.get(url, headers=self.headers) as resp:
                ...
                if resp.status < 200 or resp.status >= 300:
                    request_id = self._extract_request_id(text, resp.headers)
                    raise NewApiVideoError(
                        f"AI anime API task query failed: HTTP {resp.status} - {text}",
                        request_id=request_id,
                    )
                try:
                    return json.loads(text)
                except json.JSONDecodeError as exc:
                    raise RuntimeError(f"AI anime API task query returned invalid JSON: {text}") from exc
    ...
    async def _download_video(self, url: str, output_path: str) -> bytes:
        if url.startswith("data:"):
            ...
            async with aiohttp.ClientSession(timeout=self._client_timeout()) as session:
                async with session.get(url) as resp:
                    if resp.status != 200:
                        raise RuntimeError(f"AI anime API result download failed: HTTP {resp.status}")
    ...
    def _extract_returned_last_frame_url(task: dict) -> str:
        if not isinstance(task, dict):
            return ""
        from ai_anime.generators.huimengi import extract_huimeng_result_last_frame_url
    ...
    async def generate(
        ...
        **kwargs,
        ...
        try:
            ...
            log(f"正在提交 AI anime API 视频任务 ({model_label}, {duration}s, {request_resolution})...")
            progress(0.1)
            ...
            if not task_id:
                ...
                return VideoGenResult(
                    status=VideoGenStatus.FAILED,
                    error=f"No task_id in AI anime API response: {submitted}",
                )
            record_accepted(task_id)
            log(f"任务已提交: {task_id}")
            progress(0.2)
            ...
            for poll_count in range(max_polls):
                ...
                if status in {"completed", "succeeded", "success", "done"}:
                    ...
                    if not video_url:
                        update_request_status(task_id, "failed", "No video url in AI anime API result")
                        await _refund_video_model_call(
                            reservation_id,
                            source="newapi_video_generation",
                        ...
                        return VideoGenResult(
                            status=VideoGenStatus.FAILED,
                            error=f"No video url in AI anime API result: {task}",
                            task_id=task_id,
                        )
                    log("视频生成完成，正在下载...")
                ...
                if status in {"failed", "error", "canceled", "cancelled", "expired"}:
                    error = (
                        task.get("error") or task.get("fail_reason") or "AI anime API video task failed"
                    )
                ...
                if poll_count % 6 == 0:
                    log(
                        f"AI anime API task {task_id} status: "
                        f"{status or 'queued'} ({poll_count}/{max_polls})"
                    )
                await asyncio.sleep(poll_interval)

            update_request_status(task_id, "failed", "Timeout waiting for AI anime API video task")
            await _refund_video_model_call(
                reservation_id,
                source="newapi_video_generation",
                error="timeout",
            ...
            return VideoGenResult(
                status=VideoGenStatus.FAILED,
                error="Timeout waiting for AI anime API video task",
                task_id=task_id,
            )
        except NewApiVideoError as exc:
            if exc.request_id:
                log(f"AI anime API request_id: {exc.request_id}")
            if task_id:
                log(f"AI anime API task_id: {task_id}")
                update_request_status(task_id, "failed", str(exc))
            await _refund_video_model_call(
                reservation_id,
                source="newapi_video_generation",
...
def parse_newapi_video_backend(backend: str | None) -> str | None:
    ...
    if value == "newapi":
        from ai_anime.config import NEWAPI_VIDEO_MODEL
...
def newapi_video_backend_options(*, include_seedance2_variants: bool = False) -> dict[str, str]:
    from ai_anime.config import NEWAPI_VIDEO_MODELS
...
def create_video_generator(
    ...
    **kwargs,
    ...
    """
    from ai_anime.config import SEEDANCE_FAST_MODEL, SEEDANCE_PRO_MODEL
