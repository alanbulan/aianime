...
- 自动生成的自定义风格参考图与内置预设采用相同的“完整风格配置 + 单张成片参考图”结构；参考图可按场景要求包含人物和人脸，后续生成只迁移其视觉风格。
...
- AI-generated custom styles now mirror built-in presets with a complete style configuration plus one finished reference image; the reference may include people or faces while downstream generation transfers visual style only.
