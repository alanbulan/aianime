...
- 点击通知中心的公告可打开全文弹窗，完整显示标题和正文，保留段落与换行，长内容支持滚动阅读。
- 关闭公告详情后返回通知列表；按 Escape 优先关闭当前公告，再按一次关闭通知中心。窄窗口与深色主题均可正常阅读。
- 打开通知中心后保存当前公告的已读状态，重新打开不再重复显示红点；有新公告时继续提示。
- 铃铛只提示通知列表中的公告，避免未展示的版本更新使红点反复出现；版本更新仍使用原有更新入口。
- 包含 1.1.84 的登录状态隔离与缓存清理修复。安装更新后请完全退出并重新启动客户端。

# v1.1.84

## 登录与缓存修复

- 修复退出或切换账户时，旧请求、令牌刷新和资料加载结果覆盖当前登录状态的问题；旧请求不会再使用新账户身份重试。
- 登录状态按操作顺序保存，避免较慢的保存覆盖退出或新登录；自动登录会核对原账户与所属团队。
- 模型响应正文与调用记录分开加密保存，正文过期后自动清理，防重复调用记录继续保留；取消任务只读取该任务的记录，减少历史图片带来的等待。
- 更新后请完全退出并重新启动客户端，使登录与缓存修复生效。
