...
客户端 `1.1.67` 固定使用 `https://aianime.mingcw.com`。Windows x86_64 NSIS `1.1.6` 已作为可选更新发布，`1.1.5` 可通过现有 Gateway 完成版本判断、YAML/EXE 下载和 SHA-512 校验；后续版本继续沿用同一更新合同，客户端支持启动检查、手动检查、真实下载进度和安装确认。
