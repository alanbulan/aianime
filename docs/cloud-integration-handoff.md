...
客户端 `1.1.11` 固定使用 `https://aianime.122-193-11-199.sslip.io`。Windows x86_64 NSIS `1.1.6` 已作为可选更新发布，`1.1.5` 可通过现有 Gateway 完成版本判断、YAML/EXE 下载和 SHA-512 校验；`1.1.7` 已由发布方上传云端，`1.1.10` 已完成本地构建和制品校验，等待上传并创建新的可选更新记录。
