...
| 区域 | 当前事实 | 未满足的退出条件 |
| --- | --- | --- |
| 前端 Creative Canvas | `modules/creative_canvas` 已有 968 个 TS/TSX 文件；`features/canvas` 仍有 274 个 TS/TSX 文件和 1 个 CSS 文件，`features/freezone` 已归零并删除 | 已迁切片均由 Creative Canvas 唯一持有；第 803-838 批进一步将页面级 Surface、纯渲染投影、节点交互与共享展示基元、六类节点工具栏 View 和节点工具栏总 View 收进模块，旧 Canvas 的对应生产/测试所有者已删除。Store、旧 Canvas 内容投影、节点注册表、生成恢复、抠图节点写入/平台上传、历史素材 Viewer 和工具事件的具体适配仍只在既有组合层注入，模块不反向依赖旧 Canvas；App Shell 对旧 Canvas 的私有入口仍为 4 个，R1-C 至 R1-E 的其余 Canvas 所有权待迁移 |
...
2. R1-C 至 R1-E：旧聚合 public 与整个 Freezone 前端根已归零；继续按领域规则、应用适配器和展示出口迁移当前 `features/canvas` 的 274 个 TS/TSX 文件及 1 个样式文件，下一批先清除图片/视频节点 View 对旧 Store 的直接订阅，最后一个消费者切换后删除旧目录，不做整目录复制。
