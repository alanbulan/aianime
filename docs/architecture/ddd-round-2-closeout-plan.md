...
| 区域 | 当前事实 | 未满足的退出条件 |
| --- | --- | --- |
| 前端 Creative Canvas | `modules/creative_canvas` 已有 790 个 TS/TSX 文件；`features/canvas` 仍有 427 个 TS/TSX 文件，`features/freezone` 已归零并删除，旧 feature 合计 427 个文件 | 新模块已有真实生产所有权，Freezone 项目页与 Shell Controller/View、Mask Editor Controller/View、Canvas 入口生命周期、Shell 聊天 Dock、画布反馈、Canvas Browser、素材库完整展示链/入口/拖拽交互状态与替换 controller、素材选择弹窗完整纵向链、生成历史完整记录合同/应用查询/HTTP/Hook/组合根、历史素材 Modal 与基础资产展示链、通用图片/视频查看器状态/展示/装配与外部对话框事件链、Canvas Beat Context 引用收集/预取/项目上下文 Controller、异步节点任务去重/生成恢复 Controller、Space 平移/框选手势与命中归并、Selection Surface 组合控制器、选择同步/上传投影/删除命令、节点级联/批量删除、Viewport Bookmark 领域/相机命令/快捷键及 Minimap 可见性/按钮/书签行/菜单/覆盖层 View、触控板平移偏好、Viewport Commit/Metrics/Lifecycle/Edge Pan/Runtime Controller、Auto Layout 领域算法/控制器、Pending Node Focus/Node Focus Controller、键盘命令/Pane 右键菜单状态/命令投影/Command Surface/Menu View、节点 Hover/Placement Confirm 定时状态、泛型 Placement Controller、Pane 单/双击抑制、节点菜单快捷键/指针位置、节点点击放置/Storyboard 聚焦、节点菜单规划/Skill 初始数据、节点选择/即时生成与 Quick Add、连接预览合同/曲线路径与节点菜单状态、Skill Registry 异步加载/取消/索引投影与节点目录标签组合、Skill Catalog 领域规范化/应用端口/HTTP 缓存/组合根、捕获源与输出组联动领域规则、节点层级/位置/数据更新与深克隆/图片比例解析、生成比例吸附、路径/渲染源/base64 与节点尺寸/History/Text Annotation/Storyboard Frame/Clipboard Snapshot/Session/Hook/Duplication Planner/Controller 与平台资产迁移领域及应用规则和运行时模型空默认值、Zoom/边显隐控件与持久化状态、Storyboard Group 边恢复与 Canvas 解组、自动归组/组内排列/组边界适配、Storyboard 配置/成员布局重排/普通组转换、普通组与 Storyboard 创建/节点装配/成员追加、普通组工具栏配色/命令 controller/纯 View、Storyboard 工具栏投影/命令 controller/纯 View、Storyboard Cell Preview 媒体分类/首帧/图片回退规则、Canvas Snap Alignment 索引/吸附算法及 Hook/Store/按钮/引导线完整 presentation 链、Group Node 分镜投影/拖拽/上传/自动适配 controller 与纯 View、主线节点/边锁定与视觉状态派生、主线派生继承、受管变更过滤和边删除、素材节点生成/浏览器拖拽读取/替换拖影、媒体文件识别/交互目标/剪贴板与拖放传输 controller、音频文件/声线目录/选择/生成/分离/时长验证与面板投影、文本翻译、Story Script、视频剧情分析、视频生成模型能力/参考上限/提交完成链、视频合成核心链/输入投影/时间线领域与会话/Modal 与封面编辑/封面帧投影与浏览器截帧/完整交互展示链/媒体时长探测/预览播放与导出交付链/共享帧条捕获、字幕擦除链与 TEXT 模型目录解析、基础图片/多角度/扩图/图片高清/视频高清/重绘/全景/补光/网格模板编辑/反推提示词/图片转 3D 生成用例与提交 Gateway/唯一媒体组合根、平台图片源准备、生成结果查询唯一 adapter、生成目录领域/应用/HTTP/Hook/组合根、ImageEdit 节点纯模型、图片编辑/网格/节点/视频/管理工具栏规则、抠图结果投影、节点尺寸更新、视频元数据与浏览器拖放解析、Beat 场景素材/导演 Manifest/Palette 应用端口及两条 HTTP adapter、Director 捕获上传编排与浏览器 runtime、生成错误解析/报告与浏览器诊断 runtime、生成任务结算仲裁、节点任务状态及 Task Execution Store Hook、图片 Job 合同/轮询、跨刷新任务恢复、导出节点重试与任务查询 adapter、图片生成节点纯模型、图片节点布局与共享尺寸常量、图变更意图与节点/边副作用链，以及 CommitDialog 完整入口/控制器/视图、AB 对比和 Create Identity 对话框已迁入；项目页和最终 Shell 跨上下文装配均已上移到独立 App composition，Canvas Sync 行为与子控制器装配已进入 Creative Canvas 唯一 presentation/root composition；Freezone 前端根已无源文件和目录，App Shell 对旧 Canvas 的私有入口由 11 个收紧到 4 个；Canvas 其余所有权仍待迁移 |
...
| 架构门禁 | 残余 ratchet 已覆盖旧目录，Creative Canvas 公共入口门禁与完整后端架构门禁 184 项通过；第 792 批图变更行为 3 个文件 8 项、定向架构 2 个文件 337 项、完整前端架构目录 6 个文件 384 项、前端 TypeScript 与差异检查通过 | 后端当前检查点已全绿，前端组合根循环、十四个显式路由上下文切片、Creative Canvas 已登记切片、Freezone、AI Assistant 与 Task Execution 旧目录收敛已关闭；其余 Canvas 旧目录所有权和最终干净环境门禁仍未完成 |
...
状态：进行中。R1-A 的跨上下文装配环、R1-B 的十四个显式路由上下文切片和 R1-F 后端边界均已关闭；Freezone 前后端旧实现、旧聚合 public、runner 旧 jobs 导入、模块外 infrastructure 直连及 Canvas 生产 `readUrl()` 均为 0。前端 Creative Canvas 已持有同步、存储、Projection、提交、素材库与素材选择弹窗、项目/Shell 展示、生成历史查询、历史素材 Modal 与基础资产展示、通用图片/视频查看器状态/展示/装配与外部对话框事件、Canvas Beat Context 引用收集/预取/项目上下文 Controller、异步节点任务去重/生成恢复 Controller、Space 平移/框选手势与命中归并、Selection Surface 组合控制器、选择同步/上传投影/删除命令、节点级联/批量删除、主线节点/边锁定与视觉状态派生、主线派生继承、受管变更过滤和边删除、生成目录、文本翻译、Story Script、视频剧情分析、视频生成模型能力/参考上限/提交完成链、视频合成核心链/输入投影/时间线领域与会话/Modal 与封面编辑/封面帧投影与浏览器截帧/完整交互展示链/媒体时长探测/预览播放与导出交付链/共享帧条捕获、字幕擦除链、素材节点插入/替换拖影、媒体传输 presentation 与 Audio 领域/应用/媒体服务/生成网关及共享结果查询等已登记纵向链，项目页与 Shell 的跨上下文装配均上移到 App composition。`modules/creative_canvas`、`features/canvas` 与 `features/freezone` 当前分别有 545/637/0 个 TS/TSX 文件，App Shell 对旧 Canvas 私有入口由 11 个收紧到 4 个；R1-C 至 R1-E 的其余 Canvas 领域、应用/适配器和展示所有权仍未迁完，R1 尚未达到退出条件。
...
2. R1-C 至 R1-E：旧聚合 public 与整个 Freezone 前端根已归零；继续按领域规则、应用适配器和展示出口迁移当前 `features/canvas` 的 427 个 TS/TSX 文件，最后一个消费者切换后删除旧目录，不做整目录复制。
