...
| 区域 | 当前事实 | 未满足的退出条件 |
| --- | --- | --- |
| 前端 Creative Canvas | `modules/creative_canvas` 已有 921 个 TS/TSX 文件；`features/canvas` 仍有 312 个 TS/TSX 文件和 1 个 CSS 文件，`features/freezone` 已归零并删除 | 已迁切片均由 Creative Canvas 唯一持有；第 803-821 批进一步将 Connection Gesture、Render、Project 三个页面级 Surface、纯渲染投影、Hover Menu、框选状态投影、上游解绑、图片宫格工具栏、图片抠图运行时、图片编辑工具栏、上游引用排序/订阅、节点选择菜单、快捷添加菜单、快捷操作栏、节点框架基础基元、节点共享展示基元/样式、视频节点运镜/资产库/生成数量/生成模式/真人验证展示能力收进模块，旧 Canvas 的对应生产/测试所有者已删除。Store、旧 Canvas 内容/图片投影、节点注册表、生成恢复、抠图节点写入/平台上传、历史素材 Viewer 和工具事件的具体适配仍只在既有组合层注入，模块不反向依赖旧 Canvas；App Shell 对旧 Canvas 的私有入口仍为 4 个，R1-C 至 R1-E 的其余 Canvas 所有权待迁移 |
...
2. R1-C 至 R1-E：旧聚合 public 与整个 Freezone 前端根已归零；继续按领域规则、应用适配器和展示出口迁移当前 `features/canvas` 的 325 个 TS/TSX 文件及 1 个样式文件，最后一个消费者切换后删除旧目录，不做整目录复制。
