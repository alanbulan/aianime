...
| 区域 | 当前事实 | 未满足的退出条件 |
| --- | --- | --- |
| 前端 Creative Canvas | `modules/creative_canvas` 已有 537 个 TS/TSX 文件；`features/canvas` 仍有 644 个 TS/TSX 文件，`features/freezone` 已归零并删除，旧 feature 合计 644 个文件 | 新模块已有真实生产所有权，Freezone 项目页与 Shell Controller/View、Mask Editor Controller/View、Canvas 入口生命周期、Shell 聊天 Dock、画布反馈、Canvas Browser、素材库完整展示链/入口/拖拽交互状态与替换 controller、素材选择弹窗完整纵向链、生成历史完整记录合同/应用查询/HTTP/Hook/组合根、历史素材 Modal 与基础资产展示链、通用图片/视频查看器状态/展示/装配与外部对话框事件链、Canvas Beat Context 引用收集/预取/项目上下文 Controller、异步节点任务去重/生成恢复 Controller、Space 平移/框选手势与命中归并、Selection Surface 组合控制器、选择同步/上传投影/删除命令、主线节点/边锁定与视觉状态派生、素材节点生成/浏览器拖拽读取/替换拖影、媒体文件识别/交互目标/剪贴板与拖放传输 controller、音频文件/声线目录/选择/生成/分离/时长验证与面板投影、文本翻译、Story Script、视频剧情分析、视频生成模型能力/参考上限/提交完成链、视频合成核心链/输入投影/时间线领域与会话/Modal 与封面编辑/封面帧投影与浏览器截帧/完整交互展示链/媒体时长探测/预览播放与导出交付链/共享帧条捕获、字幕擦除链与 TEXT 模型目录解析、基础图片/多角度/扩图/图片高清/视频高清/重绘/全景/补光/网格模板编辑/反推提示词/图片转 3D 生成用例与提交 Gateway/唯一媒体组合根、平台图片源准备、生成结果查询唯一 adapter、生成目录领域/应用/HTTP/Hook/组合根，以及 CommitDialog 完整入口/控制器/视图、AB 对比和 Create Identity 对话框已迁入；项目页和最终 Shell 跨上下文装配均已上移到独立 App composition，Canvas Sync 行为与子控制器装配已进入 Creative Canvas 唯一 presentation/root composition；Freezone 前端根已无源文件和目录，App Shell 对旧 Canvas 的私有入口由 11 个收紧到 4 个，Canvas 其余所有权仍待迁移 |
...
状态：进行中。R1-A 的跨上下文装配环、R1-B 的十四个显式路由上下文切片和 R1-F 后端边界均已关闭；Freezone 前后端旧实现、旧聚合 public、runner 旧 jobs 导入、模块外 infrastructure 直连及 Canvas 生产 `readUrl()` 均为 0。前端 Creative Canvas 已持有同步、存储、Projection、提交、素材库与素材选择弹窗、项目/Shell 展示、生成历史查询、历史素材 Modal 与基础资产展示、通用图片/视频查看器状态/展示/装配与外部对话框事件、Canvas Beat Context 引用收集/预取/项目上下文 Controller、异步节点任务去重/生成恢复 Controller、Space 平移/框选手势与命中归并、Selection Surface 组合控制器、选择同步/上传投影/删除命令、主线节点/边锁定与视觉状态派生、生成目录、文本翻译、Story Script、视频剧情分析、视频生成模型能力/参考上限/提交完成链、视频合成核心链/输入投影/时间线领域与会话/Modal 与封面编辑/封面帧投影与浏览器截帧/完整交互展示链/媒体时长探测/预览播放与导出交付链/共享帧条捕获、字幕擦除链、素材节点插入/替换拖影、媒体传输 presentation 与 Audio 领域/应用/媒体服务/生成网关及共享结果查询等已登记纵向链，项目页与 Shell 的跨上下文装配均上移到 App composition。`modules/creative_canvas`、`features/canvas` 与 `features/freezone` 当前分别有 537/644/0 个 TS/TSX 文件，App Shell 对旧 Canvas 私有入口由 11 个收紧到 4 个；R1-C 至 R1-E 的其余 Canvas 领域、应用/适配器和展示所有权仍未迁完，R1 尚未达到退出条件。
...
2. R1-C 至 R1-E：旧聚合 public 与整个 Freezone 前端根已归零；继续按领域规则、应用适配器和展示出口迁移当前 `features/canvas` 的 644 个 TS/TSX 文件，最后一个消费者切换后删除旧目录，不做整目录复制。
