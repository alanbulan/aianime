...
- Production 前三批分层完成；姿势编辑扩大回归 66 项、裁剪规则/实际 PNG 写回/M05/架构门禁回归 63 项、图片设置与 Freezone/M06/M09 回归 98 项及运行时选择补充回归 11 项通过；旧 helper 引用为零，Freezone 跨路由依赖降至一处，全仓 Ruff 与 `git diff --check` 通过。
...
当前进度：进行中。前三批已建立 Production domain/application/infrastructure/composition/public 边界，并迁移草图姿势编辑、当前草图裁剪和 Render/Sketch 图片设置用例；后续按 sketch、render、audio、video、pool、export 能力继续拆分 `generation.py`，每批删除对应旧实现。
