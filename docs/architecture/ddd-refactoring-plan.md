...
| 5. Narrative Planning | 进行中 | 已建立脚本工作流、剧本文档、任务调度、Beat 视频提示词和剧集内容边界；继续迁移剧集、Seedance 提示词和前端工作台 |
...
| Narrative Planning | 首批建立后端 domain/application/infrastructure/composition/public 切片；Beat 选择、上下文和首尾帧末尾限制已从 route 下沉，脚本写作 workflow 已迁入模块；第二批迁移原文/改写稿 CRUD、改写生成与归一化规则；第三批迁移剧本读取/覆盖保存和 Beat 编辑；第四批建立 Narrative TaskScheduler 与任务 DTO，脚本生成和 Beat 提示词 payload/task key/入队响应已移出 route；旧 `ai_anime/workflows` 已删除 | 阶段 5 继续迁移 `episodes.py` 和 `scripts.py` 的 Seedance 提示词/计费编排；随后迁移前端 episode/script controller、view、gateway，并删除旧查询实现 |
