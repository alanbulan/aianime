...
| Production | 前七十二批已建立前后端 domain/application/infrastructure/composition/public 边界；后端 Production 路由已按 Settings、Audio、Export、Video、Pool、Render、Sketch 拆分，Asset & World Viewer 路由已独立，`generation.py` 已删除；前端视频配置、后端目录、视频池、网格图片池读取/重建/选图/Beat 上传/整图上传/Prompt/预览/切图、Seedance2 面板、视频生成命令、Director Control 转草图、旁白声线、剧集成片、IndexTTS2 音频生成、AudioPane controller/view、Render/Sketch 图片设置、Render Plan、草图/Render 共享生成命令、草图重生成队列、姿势编辑/裁剪及配色/AI Marker 检测数据链已迁入 Production，Beat Director Stage manifest、背景锚点、控制帧状态和资产工作台导航已迁入 Asset & World，外部调用统一依赖对应 public API，旧查询及旧 AudioPane 实现已删除 | 阶段 7 进行中；继续拆分 beat workbench 的视频与分镜 controller/view，并迁移其余旧查询能力 |
