...
| 7. Production | 进行中 | 已迁移草图编辑、图片设置、用量防护、剧集成片编排与导出、生成上下文、标记配色、AI Marker 检测与重生成队列，继续处理渲染、音频、视频与素材池 |
...
| Production | 前十一批已建立后端 domain/application/infrastructure/composition/public 边界，迁移草图姿势编辑、当前草图裁剪、Render/Sketch 图片设置、图片用量防护、剧集成片编排/状态查询/SRT/成片/ZIP 导出、生成角色上下文、草图标记颜色领域规则、显式配色、AI Marker 检测和重生成队列用例；生成、Freezone 与任务 runner 统一依赖 Production public API | 阶段 7 进行中；继续按 sketch、render、audio、video、pool 拆分 `generation.py` |
...
当前进度：进行中。前十一批已建立 Production domain/application/infrastructure/composition/public 边界，并迁移草图姿势编辑、当前草图裁剪、Render/Sketch 图片设置、图片用量防护、剧集成片编排/状态查询/SRT/成片/ZIP 导出、生成角色上下文、草图标记颜色领域规则、显式配色、AI Marker 检测和重生成队列用例；后续按 sketch、render、audio、video、pool 能力继续拆分 `generation.py`，每批删除对应旧实现。
