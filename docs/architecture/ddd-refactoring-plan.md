...
| 6. Asset & World | 进行中 | 首批收敛前端 Style 数据边界，后续迁移后端 Style 与其余资产子域 |
...
| Asset & World | 首批建立前端 domain/application/infrastructure/composition/public 切片，迁移 Style 类型、查询/缓存 hooks、HTTP gateway 和预览 URL，Style 页面、Story Intake 与项目风格标签统一通过 public API | 后端 Style 领域/应用/适配器、Style 页面 controller/view，以及 Character/Identity/Voice、Scene、Prop 仍待迁移 |
...
当前进度：进行中。首批已完成前端 Style 领域类型、查询/缓存应用层、HTTP gateway、composition/public 边界和预览 URL 迁移，3 个生产调用方已切换，旧 `lib/queries/styles.ts`、`types/style.ts`、`lib/style-preview-url.ts` 已删除。后端 Style 与前端 Style 页面拆分，以及 Character/Identity/Voice、Scene、Prop 仍待迁移。
