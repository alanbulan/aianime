...
| Production | 前六十八批已建立前后端 domain/application/infrastructure/composition/public 边界；后端 Production 路由已按 Settings、Audio、Export、Video、Pool、Render、Sketch 拆分，Asset & World Viewer 路由已独立，`generation.py` 已删除；前端视频配置、后端目录、视频池、网格图片池读取/重建/选图/Beat 上传、Seedance2 面板、视频生成命令、旁白声线、剧集成片、IndexTTS2 音频生成、Render/Sketch 图片设置、Render Plan、草图/Render 共享生成命令、草图重生成队列、姿势编辑/裁剪及配色/AI Marker 检测数据链已迁入 Production，外部调用统一依赖 public API，对应旧实现已删除 | 阶段 7 进行中；继续拆分 beat workbench 的视频、音频与分镜 controller/view，并迁移其余旧查询能力 |
