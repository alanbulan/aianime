...
| 6. Asset & World | 进行中 | Style 前后端分层、Character Voice、Character Catalog、Identity CRUD 与角色资产历史后端用例已收敛，后续迁移其余资产用例与页面 |
