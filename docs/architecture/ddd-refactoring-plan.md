...
| 7. Production | 进行中 | 已迁移草图编辑、图片设置、生成上下文、标记配色与 AI Marker 检测，继续处理渲染、音频、视频与导出 |
...
| Production | 前七批已建立后端 domain/application/infrastructure/composition/public 边界，迁移草图姿势编辑、当前草图裁剪、Render/Sketch 图片设置、生成角色上下文、草图标记颜色领域规则、显式配色和 AI Marker 检测用例；生成、Freezone 与任务 runner 统一依赖 Production public API | 阶段 7 进行中；继续按 sketch、render、audio、video、pool、export 拆分 `generation.py` |
...
当前进度：进行中。前七批已建立 Production domain/application/infrastructure/composition/public 边界，并迁移草图姿势编辑、当前草图裁剪、Render/Sketch 图片设置、生成角色上下文、草图标记颜色领域规则、显式配色和 AI Marker 检测用例；后续按 sketch、render、audio、video、pool、export 能力继续拆分 `generation.py`，每批删除对应旧实现。
