...
export function ContextOperationsPanel({
  ...
  return (
    <div className="mb-2 rounded-xl border border-warning/30 bg-warning/10 p-2">
      <div className="mb-1 flex items-center justify-between gap-2">
        <div className="text-[11px] font-medium text-warning">主线上下文</div>
        <div className="text-[10px] text-warning/85">只在身份/场景/Beat 匹配时出现</div>
      </div>
      <div className="flex flex-wrap gap-2">
        {operations.map((item) => (
          <button
            ...
            type="button"
            className="nodrag min-w-[180px] max-w-full rounded-lg border border-warning/35 bg-card px-3 py-2 text-left transition hover:bg-warning/15"
          ...
          >
            <div className="text-xs font-medium text-warning">{item.operation.label}</div>
            <div className="mt-0.5 truncate text-[10px] text-warning/85">
              {item.match.reason}
            </div>
          </button>
        ))}
