...
export interface AssetLibraryCanvasInsertionPort
  extends AssetLibraryCanvasPlacementState, CanvasAssetNodeSpawnPort {
...
export async function insertAssetLibraryAsset({
  ...
  const position = viewportCenteredPosition(canvas, index);
  ...
  const nodeId = spawnAssetNode(canvas, hydratedPayload, position);
