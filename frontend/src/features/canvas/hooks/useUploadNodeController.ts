...
export function useUploadNodeController({
  ...
  const morphToVideoWithFile = useCallback(
    (file: File) => {
      ...
      if (!converted) return;
      requestAnimationFrame(() => {
        canvasEventBus.publish('video-node/external-file', {
          nodeId: id,
          file,
        });
      });
  ...
  const morphToAudioWithFile = useCallback(
    (file: File) => {
      ...
      if (!converted) return;
      requestAnimationFrame(() => {
        canvasEventBus.publish('audio-node/external-file', {
          nodeId: id,
          file,
        });
      });
  ...
  useEffect(() =>
    canvasEventBus.subscribe('upload-node/reupload', ({ nodeId }) => {
      if (nodeId === id) {
  ...
  useEffect(() =>
    canvasEventBus.subscribe('upload-node/paste-image', ({ nodeId, file }) => {
  ...
  useEffect(() =>
    canvasEventBus.subscribe('upload-node/external-file', ({ nodeId, file }) => {
      if (nodeId === id) {
...
export type UploadNodeController = ReturnType<typeof useUploadNodeController>;
