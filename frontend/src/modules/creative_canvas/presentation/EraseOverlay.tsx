  ...
  return memo(({
    ...
    const { onPointerDown, onPointerMove, onPointerUp, recomputeHasMask } =
      useMaskPainting({
        ...
        tool,
        brushSize,
        enabled: !submitting && Boolean(imageDims),
    ...
    // 视觉模型读取 RGB；导出源图并用均匀半透明红标注编辑区域。
    const buildMaskBlob = useCallback(async (): Promise<Blob> => {
      const mask = maskCanvasRef.current;
      const sourceImage = sourceImageRef.current;
      if (!mask) throw new Error('mask canvas not ready');
      if (!sourceImage) throw new Error('source image not ready');
      return await buildRedHighlightMaskBlob(sourceImage, mask);
    }, []);
