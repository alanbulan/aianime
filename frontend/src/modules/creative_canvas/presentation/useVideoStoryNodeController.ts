...
export interface VideoStoryNodeStore {
  setSelectedNode: (id: string) => void;
  updateNodeData: (
    id: string,
    patch: Partial<VideoStoryNodeData>,
  ) => void;
}

export type VideoStoryNodeStoreHook = <TSelected>(
  selector: (state: VideoStoryNodeStore) => TSelected,
) => TSelected;
...
export interface VideoStoryNodeController {
  id: string;
  selected?: boolean;
  title: string;
  size: {
  ...
  rows: VideoStoryRow[];
  status: 'analyzing' | 'error' | 'ready' | 'empty';
  errorMessage: string;
...
export function createUseVideoStoryNodeController({
  useStore,
}: {
  useStore: VideoStoryNodeStoreHook;
}) {
  return function useVideoStoryNodeController({
    id,
    data,
    selected,
    width,
    height,
  }: VideoStoryNodeControllerOptions): VideoStoryNodeController {
