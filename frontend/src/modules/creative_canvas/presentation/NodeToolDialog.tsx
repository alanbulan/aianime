...
import { getToolPlugin } from '../domain/canvasToolRegistry';
import { inheritMainlineFields } from '../domain/inheritMainlineFields';
import {
  NODE_TOOL_TYPES,
  type CanvasToolDialogRequest,
  type NodeToolType,
...
export interface NodeToolDialogStore {
  activeToolDialog: CanvasToolDialogRequest | null;
  nodes: CanvasNode[];
...
export type NodeToolDialogStoreHook = <TSelected>(
  selector: (state: NodeToolDialogStore) => TSelected,
) => TSelected;

export type NodeToolDialogProcessTool = (
  toolType: NodeToolType,
  sourceImageUrl: string,
  options: Record<string, unknown>,
) => Promise<CanvasToolResult>;

export type NodeToolDialogPrepareNodeImage = (
  imageUrl: string,
  maxPreviewDimension?: number,
) => Promise<PreparedNodeImage>;

export type NodeToolDialogUploadLocalImage = (
  projectId: string,
  localImageUrl: string,
  filename: string,
) => Promise<string>;

export type NodeToolDialogReadStoryboardMetadata = (
  source: string,
...
export function createNodeToolDialog({
  ...
  useStore: NodeToolDialogStoreHook;
  closeToolDialog: () => void;
  processTool: NodeToolDialogProcessTool;
  prepareNodeImage: NodeToolDialogPrepareNodeImage;
  uploadLocalImageToBackend: NodeToolDialogUploadLocalImage;
  readStoryboardImageMetadata: NodeToolDialogReadStoryboardMetadata;
}) {
  return function NodeToolDialog({ projectId }: NodeToolDialogProps) {
    const { t } = useTranslation();
    const activeToolDialog = useStore((state) => state.activeToolDialog);
    const nodes = useStore((state) => state.nodes);
    const addDerivedExportNode = useStore((state) => state.addDerivedExportNode);
    const addStoryboardSplitNode = useStore((state) => state.addStoryboardSplitNode);
    ...
    const activePlugin = useMemo<CanvasToolPlugin | null>(() => {
      if (!displayToolDialog) {
        return null;
      }
...
export type NodeToolDialog = ReturnType<typeof createNodeToolDialog>;
