...
const SHORTCUT_GROUPS: ShortcutGroup[] = [
  {
    ...
    rows: [
      ...
      {
        labelKey: 'canvas.shortcuts.actions.duplicateNode',
        keys: ['⌥'],
        noteKey: 'canvas.shortcuts.gestures.dragNode',
      },
      {
        labelKey: 'canvas.shortcuts.actions.multiSelect',
        keys: ['⌘'],
        noteKey: 'canvas.shortcuts.gestures.clickNode',
      },
  ...
  },
  {
    ...
    rows: [
      ...
      {
        labelKey: 'canvas.shortcuts.actions.boxSelect',
        noteIcons: [MouseLeft],
              ...
              {group.rows.map((row, rowIndex) => (
                ...
                >
                  ...
                  <span className="flex shrink-0 items-center gap-1">
                    {row.keyKeys?.map((key, index) => (
                      <Keycap key={`k-${index}`} token={t(key)} />
                    ))}
                    ...
                    {row.noteIcons ? (
                      ...
                      >
                        {row.noteIcons.map((Icon, index) => (
                          <Icon
                            key={index}
                            className={row.noteIconClassName ?? 'h-[18px] w-[18px]'}
                          />
                        ))}
                      </span>
                    ) : row.noteKey ? (
