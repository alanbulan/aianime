...
import { collectBatchDeletableIds } from '../domain/canvasNodeDeletion';
import { computeAutoLayout } from '../domain/canvasAutoLayout';
import { DEFAULT_CANVAS_NODE_WIDTH } from '../domain/canvasGeometry';
import type { CanvasNode, CanvasEdge } from '../domain/canvasNodeData';
...
export interface MultiSelectionToolbarStore {
  nodes: CanvasNode[];
  edges: CanvasEdge[];
  setNodePositions: (
...
export type MultiSelectionToolbarStoreHook = <TSelected>(
  selector: (state: MultiSelectionToolbarStore) => TSelected,
) => TSelected;
...
export function createMultiSelectionToolbar({
  useStore,
}: {
  useStore: MultiSelectionToolbarStoreHook;
}) {
  return memo(() => {
    const nodes = useStore((state) => state.nodes);
    const edges = useStore((state) => state.edges);
    const setNodePositions = useStore((state) => state.setNodePositions);
    const duplicateNodesAsSiblings = useStore(
    ...
    const mergeStoryboardGroup = useStore((state) => state.mergeStoryboardGroup);
...
export type MultiSelectionToolbar = ReturnType<typeof createMultiSelectionToolbar>;
