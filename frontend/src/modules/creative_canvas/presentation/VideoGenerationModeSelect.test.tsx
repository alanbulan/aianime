...
describe("VideoGenerationModeSelect", () => {
  it("keeps the active mode available and routes another selection", async () => {
    ...
    await clickAndLayout(screen.getByRole("button", { name: "文生视频" }));
    expect(onParentClick).not.toHaveBeenCalled();
    expect(screen.getAllByRole("button", { name: "文生视频" })[1]).toBeEnabled();

    await clickAndLayout(screen.getByRole("button", { name: "全能参考" }));
  ...
  it("shows the supplied reason while hovering a disabled mode", async () => {
    render(
      <VideoGenerationModeSelect
        value="allReference"
        options={options}
  ...
  it("renders the portal and closes it on an outside pointer", async () => {
    ...
    expect(popover.closest("[data-ui-overlay]")).not.toBeNull();
