...
import { validMainlineContexts } from '../domain/mainlineContext';
import {
...
} from '../application/directorCaptureBundle';
import type { GetCanvasBeatDirectorManifestParams } from '../application/beatDirectorManifest';
import type { GetCanvasDirectorStagePaletteParams } from '../application/directorStagePalette';
import type {
...
export interface ThreeDWorldNodeCanvasNode {
  id: string;
  type?: string | null;
...
export interface ThreeDWorldNodeStore {
  setSelectedNode: (id: string | null) => void;
  updateNodeData: (id: string, patch: Partial<ThreeDWorldNodeData>) => void;
  addPanoCaptureGroup: (
    sourceNodeId: string,
    captures: Array<{
      dataUrl: string;
...
export type ThreeDWorldNodeStoreHook = <TSelected>(
  selector: (state: ThreeDWorldNodeStore) => TSelected,
) => TSelected;

export type ThreeDWorldNodeUpstreamNodes = (
  nodeId: string,
) => ThreeDWorldNodeCanvasNode[];

export type ThreeDWorldNodeDetachUpstream = (
  nodeId: string,
) => (sourceNodeId: string) => void;

export type ThreeDWorldNodeGenerateImageTo3d = (
...
export type ThreeDWorldNodeReadNode = (
  nodeId: string,
) => ThreeDWorldNodeCanvasNode | undefined;

export type ThreeDWorldNodeGetBeatManifest = (
  params: GetCanvasBeatDirectorManifestParams,
) => Promise<DirectorStageManifest>;

export type ThreeDWorldNodeGetDirectorPalette = (
  params: GetCanvasDirectorStagePaletteParams,
) => Promise<DirectorStageManifest['palette']>;

export type ThreeDWorldNodeUploadCanvasAsset = (
  projectId: string,
  file: File | Blob,
  filename: string,
...
export type ThreeDWorldNodeUploadLocalImage = (
  projectId: string,
  dataUrl: string,
  filename: string,
) => Promise<string>;

export type ThreeDWorldNodeCaptureBlobToDataUrl = (
  blob: Blob,
  message?: string,
) => Promise<string>;

export type ThreeDWorldNodeReadCaptureSize = (
  dataUrl: string,
  message?: string,
...
export function createUseThreeDWorldNodeController({
  useStore,
  useUpstreamNodes,
  useDetachUpstream,
  ...
  useStore: ThreeDWorldNodeStoreHook;
  useUpstreamNodes: ThreeDWorldNodeUpstreamNodes;
  useDetachUpstream: ThreeDWorldNodeDetachUpstream;
  generateCanvasImageTo3d: ThreeDWorldNodeGenerateImageTo3d;
  getCanvasBeatDirectorManifest: ThreeDWorldNodeGetBeatManifest;
  getCanvasDirectorStagePalette: ThreeDWorldNodeGetDirectorPalette;
  uploadAndAutoCommitSelectedBackgroundCandidate: (
    projectId: string,
  ...
  uploadCanvasAsset: ThreeDWorldNodeUploadCanvasAsset;
  uploadLocalImageToBackend: ThreeDWorldNodeUploadLocalImage;
  directorCaptureBlobToDataUrl: ThreeDWorldNodeCaptureBlobToDataUrl;
  readDirectorCaptureImageSize: ThreeDWorldNodeReadCaptureSize;
  readNode: ThreeDWorldNodeReadNode;
}) {
  ...
  return function useThreeDWorldNodeController({
    id,
    data,
    selected,
    width,
    ...
    const submitGeneration = useCallback(async () => {
      ...
      try {
        ...
        const currentWorld = readNode(id);
      ...
      async (snapshot: ThreeDSceneSnapshot, activeSourceId?: string) => {
        updateNodeData(
          ...
          buildThreeDWorldSaveScenePatch(
            ...
            snapshot,
            activeSourceId,
          ) as Partial<ThreeDWorldNodeData>,
        );
      },
...
export type ThreeDWorldNodeController = ReturnType<
  ReturnType<typeof createUseThreeDWorldNodeController>
>;
