// Copyright (c) 2026 AI anime
import { useSyncExternalStore } from "react";

import {
  listCanvasVideoCameraTemplates,
  type CanvasGenerationCatalogGateway,
} from "../application/generationCatalog";
import type { CameraMovementPreset } from "../domain/cameraMovementPresets";

export interface UseCanvasVideoCameraTemplatesResult {
  templates: CameraMovementPreset[];
  isLoading: boolean;
  error: Error | null;
}

export function createCanvasVideoCameraTemplateHooks(
  gateway: CanvasGenerationCatalogGateway,
) {
  const EMPTY: UseCanvasVideoCameraTemplatesResult = {
    templates: [],
    isLoading: false,
    error: null,
  };

  // Per-project shared store for authenticated camera templates.
  const states = new Map<string, UseCanvasVideoCameraTemplatesResult>();
  const listeners = new Map<string, Set<() => void>>();

  function emit(project: string) {
    listeners.get(project)?.forEach((fn) => fn());
  }

  function writeState(project: string, next: UseCanvasVideoCameraTemplatesResult) {
    states.set(project, next);
    emit(project);
  }

  function ensureLoaded(project: string) {
    if (states.has(project)) return;
    states.set(project, { templates: [], isLoading: true, error: null });
    listCanvasVideoCameraTemplates(project, gateway)
      .then((templates) => {
        writeState(project, { templates, isLoading: false, error: null });
      })
      .catch((error: unknown) => {
        const normalized =
          error instanceof Error ? error : new Error(String(error));
        console.warn(
          "[creative-canvas] video/camera-templates fetch failed:",
          normalized.message,
        );
        writeState(project, { templates: [], isLoading: false, error: normalized });
      });
  }

  function prefetchCanvasVideoCameraTemplates(project: string): void {
    if (!project) return;
    ensureLoaded(project);
  }

  function subscribe(project: string | null, callback: () => void) {
    if (!project) return () => { };
    let bucket = listeners.get(project);
    if (!bucket) {
      bucket = new Set();
      listeners.set(project, bucket);
    }
    bucket.add(callback);
    return () => {
      bucket!.delete(callback);
      if (bucket!.size === 0) listeners.delete(project);
    };
  }

  function useCanvasVideoCameraTemplates(
    project: string | null,
  ): UseCanvasVideoCameraTemplatesResult {
    if (project) ensureLoaded(project);

    return useSyncExternalStore(
      (callback) => subscribe(project ?? null, callback),
      () => (project ? states.get(project) ?? EMPTY : EMPTY),
      () => EMPTY,
    );
  }

  return {
    prefetchCanvasVideoCameraTemplates,
    useCanvasVideoCameraTemplates,
  };
}
