...
import { isScriptNode, isVideoNode } from '../domain/canvasNodePredicates';
import type { CanvasNode, CanvasNodeType } from '../domain/canvasNodeData';

export interface NodeSpawnPlusOverlayStore {
  nodes: CanvasNode[];
  activeOverlayNodeId: string | null;
}

export type NodeSpawnPlusOverlayStoreHook = <TSelected>(
  selector: (state: NodeSpawnPlusOverlayStore) => TSelected,
) => TSelected;
...
export function createNodeSpawnPlusOverlay({
  useCanvasStore,
}: {
  useCanvasStore: NodeSpawnPlusOverlayStoreHook;
}) {
  return memo(({
    hoveredNodeId = null,
    hidden = false,
...
export type NodeSpawnPlusOverlay = ReturnType<typeof createNodeSpawnPlusOverlay>;
