...
import type { MainlineContext } from "@/modules/creative_canvas/domain/mainlineContext";
...
export type AssetLibraryDropMediaType = "image" | "video" | "audio" | "model";
...
export function assetDropMediaType(
  asset: LibraryAsset,
): AssetLibraryDropMediaType | null {
...
function createSceneDirectorWorldAsset(
  ...
  const rawSources = sourceAssets
    ...
    .filter((source): source is DirectorWorldSourceDescriptor => source !== null);
...
function directorWorldSourceFromSceneAsset(
  sceneId: string,
  asset: LibraryAsset,
): DirectorWorldSourceDescriptor | null {
...
function sceneDirectorSourceKind(
  role: string,
): NonNullable<DirectorWorldSourceDescriptor["source_kind"]> | null {
...
function directorWorldSourceUrl(
  source: DirectorWorldSourceDescriptor | undefined,
): string | null {
  return source?.ply_url ?? source?.pano_url ?? source?.url ?? null;
}

function sourceKindLabel(
  source: Pick<DirectorWorldSourceDescriptor, "source_kind" | "source_type">,
): string {
