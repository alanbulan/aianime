...
function projectionKeyFromNode(node: ProjectionGraphNode): string | null {
...
function projectionKeyFromEdge(edge: ProjectionGraphEdge): string | null {
...
export function scopeProjectionGraphIds<
  Node extends ProjectionGraphNode,
  Edge extends ProjectionGraphEdge,
>(
  rawNodes: Node[],
  rawEdges: Edge[],
): { nodes: Node[]; edges: Edge[] } {
  const explicitProjectionKeyByNode = new WeakMap<Node, string>();
  ...
  const projectionKeyByNode = new WeakMap<Node, string | null>();
  const resolveNodeProjectionKey = (node: Node): string | null => {
  ...
  const inferEdgeProjectionKey = (edge: Edge): string | null => {
