...
function contextPriority(context: MainlineContext): number {
  if (context.kind === "beat") return 0;
  if (
    context.kind === "sketch" ||
    context.kind === "frame" ||
    context.kind === "video" ||
    context.kind === "audio" ||
    context.kind === "director_combined" ||
    context.kind === "selected_background"
  ) {
    return 1;
  }
  if (context.kind === "identity") return 2;
  if (context.kind === "scene") return 3;
  if (context.kind === "prop") return 4;
  return 5;
}
...
export function validMainlineContexts(contexts?: unknown): MainlineContext[] {
  return Array.isArray(contexts)
    ? contexts
        .filter(isMainlineContext)
        .sort((left, right) => contextPriority(left) - contextPriority(right))
    : [];
}
...
export function getMainlineEdgeKind(
  edge: MainlineContextEdgeLike,
): MainlineEdgeKind | undefined {
...
export function isPropagatingMainlineEdge(
  edge: MainlineContextEdgeLike,
): boolean {
  const data = edge.data;
