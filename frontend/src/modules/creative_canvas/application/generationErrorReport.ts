...
import type { GenerationRuntimeDiagnostics } from "./generationRuntime";
...
export interface BuildGenerationErrorReportInput {
  errorMessage: string;
  errorDetails?: string;
  context?: unknown;
}
...
export function buildGenerationErrorReport(
  ...
  sections.push(
    `- OS: ${context.osName ?? "Unknown"} ${context.osVersion ?? "unknown"}`.trim(),
  );
  ...
  if (
    Array.isArray(context.referenceImagePlaceholders) &&
    context.referenceImagePlaceholders.length > 0
  ) {
    sections.push(
      `- Reference Image Placeholders: ${context.referenceImagePlaceholders.join(", ")}`,
    );
  }
  sections.push("");
