...
import { httpFreezoneCanvasStorageGateway } from "./httpFreezoneCanvasStorageGateway";

vi.mock("@/shared/api/client", () => ({
  apiCall: vi.fn(),
}));

describe("httpFreezoneCanvasStorageGateway", () => {
  beforeEach(() => {
  ...
  it("lists canvases and forwards cancellation", async () => {
    ...
    await httpFreezoneCanvasStorageGateway.listCanvases({
      projectId: "project/a",
  ...
  it("reads an encoded canvas path and forwards cancellation", async () => {
    ...
    await httpFreezoneCanvasStorageGateway.getCanvas({
      projectId: "project/a",
      canvasId: "user eric",
      signal: controller.signal,
    });
  ...
  it("posts the preset payload through the canonical endpoint", async () => {
    ...
    await httpFreezoneCanvasStorageGateway.createFromPreset({
      projectId: "project-a",
      payload,
    });
  ...
  it("persists canvas changes with a PUT request body", async () => {
    ...
    await httpFreezoneCanvasStorageGateway.saveCanvas({
      projectId: "project-a",
      canvasId: "user_eric",
      payload,
    });
  ...
  it("sends unload saves through the encoded keepalive endpoint", () => {
    ...
    httpFreezoneCanvasStorageGateway.saveCanvasKeepalive({
      projectId: "project/a",
      canvasId: "user eric",
      payload,
    });
  ...
  it("owns canvas deletion transport", async () => {
    ...
    await httpFreezoneCanvasStorageGateway.deleteCanvas({
      projectId: "project-a",
      canvasId: "story-lab",
    });
    expect(apiCall).toHaveBeenCalledWith(
