...
const { isAllowedScriptDragItem, isAllowedScriptUpload } = vi.hoisted(() => ({
  isAllowedScriptDragItem: vi.fn(),
  isAllowedScriptUpload: vi.fn(),
}));

vi.mock("@/features/superchat/ingest-automation-domain", () => ({
...
describe("SuperChat Composer attachments controller", () => {
  beforeEach(() => {
    ...
    isAllowedScriptDragItem.mockReset();
    isAllowedScriptUpload.mockReset();
  });
  ...
  it("reads allowed files into attachments and supports remove and clear", () => {
    isAllowedScriptUpload.mockImplementation(
  ...
  it("tracks nested drag depth, validity, drop effect, and accepted drops", () => {
    isAllowedScriptUpload.mockReturnValue(true);
    isAllowedScriptDragItem.mockImplementation(
      (item: { type?: string }) => item.type === "text/plain",
    );
    const { result } = renderHook(() =>
      useComposerAttachmentsController(true),
    );
