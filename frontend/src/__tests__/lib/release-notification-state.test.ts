...
describe("release notification local state", () => {
  ...
  it("keeps current-release and upgrade keys separate", () => {
    expect(releaseSeenKey("v1.0.5")).toBe("ai-anime:release-seen:v1.0.5");
    expect(releaseUpgradeKey("v1.0.5")).toBe("ai-anime:release-upgrade:v1.0.5");
